...
- **Design spec:** see `STAGING_SPEC.md` — `Move-StagedDuplicatesToDeleteReview.ps1` v0.2.2 delete-review move Execute hardening (MOVE-only; full execute preflight; mandatory inventory coupling; `-Execute` not approved); `Remove-StagedDuplicateCandidates.ps1` v0.2.0 parked as final-delete tool only
...
**Safe next step:** HIGH-confidence junk delete-review move DryRun available via `Move-StagedDuplicatesToDeleteReview.ps1` v0.2.2 (execute preflight hardened). **Do not run delete-review `-Execute` or hard-delete `-Execute`.** Pending Codex re-review and Jim approval.

#### 6b. Move staging Codex Sparky review (2026-07-03)

**Script:** `Move-RecoveredToRealnameMatches.ps1` v0.1.0 → v0.1.4
