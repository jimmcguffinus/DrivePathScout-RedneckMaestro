...
param(
    ...
    [string]$ExpectedPlanHash = '71951A0E058C19A57B53FD0876BA6FEA421BFAA7DA5C248FA2F40E8EC559426D',
...
$planPath = Join-Path $ReportRoot "approved_phase2_mcnasbackup_music_human_move_extras_plan_$Stamp.csv"
$manifestPath = Join-Path $ReportRoot "phase2_mcnasbackup_music_human_move_extras_manifest_$Stamp.csv"
$journalPath = Join-Path $ReportRoot "phase2_mcnasbackup_music_human_move_extras_journal_$Stamp.csv"
$keeperPolicyPath = Join-Path $ReportRoot "phase2_mcnasbackup_music_keeper_policy_$Stamp.csv"
$inventoryPath = Join-Path $ReportRoot "phase2_mcnasbackup_music_duplicate_inventory_$Stamp.csv"
$verificationOut = Join-Path $ReportRoot "phase2_mcnasbackup_music_human_move_extras_verification_$Stamp.txt"
$snapshotCsv = Join-Path $ReportRoot "workbench_after_phase2_music_extras_snapshot_$Stamp.csv"
$snapshotSummary = Join-Path $ReportRoot "workbench_after_phase2_music_extras_snapshot_summary_$Stamp.txt"

$musicExtrasRoot = 'I:\_RECOVERY_WORKBENCH\06_HUMAN_REVIEW\media\music\mcnasbackup_duplicate_extras'
$humanReviewRoot = 'I:\_RECOVERY_WORKBENCH\06_HUMAN_REVIEW'
$deleteReviewRoot = 'I:\_RECOVERY_WORKBENCH\05_DELETE_REVIEW'
$audioExt = @('.mp3', '.m4a', '.wma', '.wav', '.flac', '.aac', '.ogg')
$expectedCount = 2838
...
Add-Check 'PlanRowCount' ($plan.Count -eq $expectedCount) "plan=$($plan.Count) expected=$expectedCount"
Add-Check 'ManifestRowCount' ($manifest.Count -eq $expectedCount) "manifest=$($manifest.Count)"
...
Add-Check 'JournalMovedVerified' ($movedVerified.Count -eq $expectedCount) "count=$($movedVerified.Count)"
Add-Check 'JournalMoveFailed' ($moveFailed.Count -eq 0) "failed=$($moveFailed.Count)"
Add-Check 'ManifestMovedVerified' ($manifestMoved.Count -eq $expectedCount) "count=$($manifestMoved.Count)"
...
foreach ($row in $plan) {
    $i++
    if ($i % 250 -eq 0) { Write-Host "  verify plan $i / $($plan.Count)" }
    ...
    if (-not (Test-PathInsideRoot -ChildPath $dest -RootPath $musicExtrasRoot)) { $destOutsideLane++ }
...
$destFiles = @(Get-ChildItem -LiteralPath $musicExtrasRoot -File -ErrorAction SilentlyContinue)
Add-Check 'DestExtrasFileCount' ($destFiles.Count -eq $expectedCount) "count=$($destFiles.Count)"

$badDestExt = @($destFiles | Where-Object { $audioExt -notcontains $_.Extension.ToLowerInvariant() })
Add-Check 'DestAudioExtensionsOnly' ($badDestExt.Count -eq 0) "badExt=$($badDestExt.Count)"
...
foreach ($kv in $keeperByHash.Values) {
    $ki++
    if ($ki % 100 -eq 0) { Write-Host "  verify keeper $ki / $($keeperByHash.Count)" }
    $kp = $kv.KeeperPath
...
$holdLanes = @{
    ...
    'MEDIUM_REVIEW_MOVE_EXTRAS_READY' = @($keeperPolicy | Where-Object ProposedFutureLane -eq 'MEDIUM_REVIEW_MOVE_EXTRAS_READY')
...
$musicExtrasCount = @($snapshotRows | Where-Object {
    Test-PathInsideRoot -ChildPath $_.FullPath -RootPath $musicExtrasRoot
}).Count
$musicExtrasBytes = ($snapshotRows | Where-Object {
    Test-PathInsideRoot -ChildPath $_.FullPath -RootPath $musicExtrasRoot
...
$lines = @(
    'Phase 2 McNASBackup Music Human Move-Extras Verification Receipt'
    ...
    'PlanRows=2838 MovedVerified=2838 MoveFailed=0 TotalBytes=9323075199'
    ''
    '=== CHECKS ==='
)
$lines += $checks
...
$lines += ''
$lines += '=== DESTINATION ==='
$lines += "music_extras_root=$musicExtrasRoot"
$lines += "music_extras_file_count=$($destFiles.Count)"
$lines += "music_extras_total_bytes=$musicExtrasBytes"
$lines += ''
$lines += '=== KEEPER SAFETY ==='
...
$lines += "MEDIUM_REVIEW_MOVE_EXTRAS_READY_groups=$($holdLanes['MEDIUM_REVIEW_MOVE_EXTRAS_READY'].Count)"
...
$sumLines = @(
    'Workbench Snapshot After Phase 2 McNASBackup Music Move-Extras Execute'
    "GeneratedUtc: $((Get-Date).ToUniversalTime().ToString('o'))"
    ...
    ''
    "MusicExtrasFiles: $musicExtrasCount"
    "MusicExtrasBytes: $musicExtrasBytes"
