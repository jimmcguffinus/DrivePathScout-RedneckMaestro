...
$script:ReviewLaneBySourceSubfolder = @{
    'images\png\browser_extension_assets' = 'high_confidence_junk\browser_extension_assets'
    'images\png\theme_assets'             = 'high_confidence_junk\theme_assets'
}
...
function Read-ApprovedReviewMovePlan {
    ...
    $required = @('Hash', 'ShortHash', 'SizeBytes', 'StagedDuplicatePath', 'DeleteReviewPath', 'KeeperPath', 'DestinationSubfolder', 'DeleteConfidence', 'DeleteReason')
    ...
    foreach ($row in $rows) {
        ...
        if ($confidence -ne 'HIGH') {
            throw "Approved review move plan row is not HIGH confidence: $stagedPath"
        }
        ...
        if ($stagedPath.Equals($keeperPath, [StringComparison]::OrdinalIgnoreCase)) {
            throw "Approved review move plan must not target keeper path as staged duplicate: $stagedPath"
        }
...
$preflightReport = Join-Path $reportRootNormalized "delete_review_move_preflight_$RunStamp.csv"
