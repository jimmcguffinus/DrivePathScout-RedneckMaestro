...
`Move-RecoveredToRealnameMatches.ps1` v0.1.2 implements move-only staging **preflight** per `STAGING_SPEC.md`. Default mode is DryRun — it does not move, copy, delete, or rename files.
...
`-Execute` is not approved. v0.1.2 tightens execute gates (full path-chain reparse checks, `ReportRoot` must be outside `I:\`, complete execution manifest, `BEFORE_MOVE` only after `ShouldProcess`). DryRun planning passed Codex Sparky review; Execute remains blocked pending re-review and Jim approval. v0.1 approved recovered move sources: `I:\recover\` only.
