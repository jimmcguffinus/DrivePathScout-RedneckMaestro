...
param(
    ...
    [string]$KeeperPolicyPath = 'C:\Users\jim\Desktop\DrivePathInventory\phase2_mcnasbackup_music_keeper_policy_20260704.csv',

    [ValidateNotNullOrEmpty()]
    [string]$InventoryPath = 'C:\Users\jim\Desktop\DrivePathInventory\phase2_mcnasbackup_music_duplicate_inventory_20260704.csv',
    ...
    [ValidateNotNullOrEmpty()]
    [string]$HumanReviewMusicExtrasRoot = 'I:\_RECOVERY_WORKBENCH\06_HUMAN_REVIEW\media\music\mcnasbackup_duplicate_extras',

    [switch]$SkipLiveHashVerification
)

# Phase 2 McNASBackup music human move-extras approved plan builder v0.3.0
...
$allowedExtensions = @('.mp3', '.m4a', '.wma', '.wav', '.flac', '.aac', '.ogg')
$deniedInventoryLanes = @('LOW_RISK_AUDIO_CACHE_OR_SAMPLE', 'BLOCKED_INVESTIGATE')
...
$reviewRoot = Get-NormalizedPath $HumanReviewMusicExtrasRoot
...
if (-not (Test-PathInsideRoot -ChildPath $reviewRoot -RootPath $humanReviewRoot)) {
    throw "Human review music extras root must be under 06_HUMAN_REVIEW: $reviewRoot"
}
if (Test-PathInsideRoot -ChildPath $reviewRoot -RootPath $deleteReviewRoot) {
    throw "Human review music extras root must not be under 05_DELETE_REVIEW: $reviewRoot"
}
...
$excludedCounts = [ordered]@{
    ...
    MediumReviewMoveExtrasReady = @($keeperPolicy | Where-Object ProposedFutureLane -eq 'MEDIUM_REVIEW_MOVE_EXTRAS_READY').Count
...
foreach ($group in $eligibleGroups) {
    ...
    $riskLevel = if ($group.PersonalLibrarySignal -eq 'True') { 'HIGH' } else { 'MEDIUM' }
    ...
    foreach ($invRow in $groupInventory) {
        ...
        $rowNum++
        if ($rowNum % 250 -eq 0) { Write-Host "  validate $rowNum" }
        ...
        if ($proposedLane -in @('HUMAN_REVIEW_SAMPLE_FIRST', 'MEDIUM_REVIEW_SAMPLE_FIRST', 'MEDIUM_REVIEW_MOVE_EXTRAS_READY', 'LOW_RISK_HOLD', 'BLOCKED_INVESTIGATE')) {
            [void]$blockReasons.Add('ExcludedFutureLane')
        }
        ...
        if ($invRow.SuggestedPolicyLane -in $deniedInventoryLanes) { [void]$blockReasons.Add('WrongInventoryLane') }
...
Add-BadTest 'SourceOutsideMcNas' {
    if (Test-PathInsideRoot -ChildPath 'I:\recover\McNASBackup\test.mp3' -RootPath $mcNasRoot) { throw 'reject' }
    throw 'accept'
}
Add-BadTest 'SourceUnderWorkbench' {
    if (Test-UnderWorkbenchRoot -Path 'I:\_RECOVERY_WORKBENCH\06_HUMAN_REVIEW\test.mp3') { throw 'reject' }
    throw 'accept'
}
...
Add-BadTest 'DestinationUnderDeleteReview' {
    $bad = Join-Path $deleteReviewRoot 'media\music\test.mp3'
...
$planPath = Join-Path $ReportRoot "approved_phase2_mcnasbackup_music_human_move_extras_plan_$PlanStamp.csv"
$shaPath = Join-Path $ReportRoot "approved_phase2_mcnasbackup_music_human_move_extras_plan_$PlanStamp.sha256.txt"
...
Write-Host "McNASBackup music human move-extras plan rows: $($planRows.Count)"
...
Write-Host "Excluded MediumReviewMoveExtrasReady=$($excludedCounts.MediumReviewMoveExtrasReady)"
